...
"""Threaded versions of draw functions."""
...
def run_with_threads(tur, log, main_turtle):
   ...
   #Lock is created here.
    lock = threading.Lock()

    #Each thread is created here.
    tCircle = threading.Thread(target=draw_circles_thread, args=(tur, lock,))
    tSquare = threading.Thread(target=draw_squares_thread, args=(tur, lock,))
    tTriangle = threading.Thread(target=draw_triangles_thread, args=(tur, lock,))
    tRectangle = threading.Thread(target=draw_rectangles_thread, args=(tur, lock,))
    ...
    #Lock inside each draw function (1 variable that it's called in argument)
